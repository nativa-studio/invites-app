-- The order of the good to know lines.
--
-- The lines are built from separate fields, so their order was the order the code happened to
-- check those fields in. That is a reasonable default and a bad rule: a host whose party hangs on
-- "come pool ready and sun smart" cannot put it above the line about gifts.
--
-- Stored as the list of kinds rather than a number per line, because a list cannot end up with two
-- things claiming the same position. Each kind appears at most once in the list the code builds,
-- so a kind is a safe key.
--
-- An empty list means the default order, so nothing is backfilled and an event created before this
-- behaves exactly as it did.

alter table public.events
  add column if not exists know_order text[] not null default '{}';

-- The function below is 0008's, unchanged except for know_order in the chained object that carries
-- the show switches. It is copied rather than retyped: the first object is already at Postgres's
-- ceiling of 100 arguments, which is why the fields are chained across several objects in the
-- first place.
