-- A second person to text about the event.
--
-- One name and one number meant a host with two of them typed both names into the one box:
-- "Text Marcia or Tommy 0403 692 420", which is one number for two people and a guest guessing
-- which of them it belongs to. Two names and two numbers, each its own line and each its own tap,
-- so pressing a name opens a message to that person.
alter table public.events add column if not exists ask_name_2 text;
alter table public.events add column if not exists ask_phone_2 text;

-- And out to the guests, which is the half that gets forgotten. A column hosts can set and
-- guests never receive saves, says it saved, and changes nothing, and that has now happened to
-- four columns in one day. The list below is an allowlist on purpose: forgetting one breaks a
-- feature loudly enough to be found, where a denylist would leak a private column silently.
-- supabase/checks/public-json-coverage.sql names anything on events that is not in here.
