-- Who a guest texts, kept apart from who signs off.
--
-- One column did both jobs. host_line is the sign-off, "With love Gabe, Tommy and Ma", and the
-- Questions block built its line from the same words, so the invite read "Text Gabe, Tommy and
-- Ma" when the person who actually answers the phone is Marcia. They are two different facts:
-- one is who the invite is from, the other is who to ring about it.
--
-- Null means the old behaviour, the name out of the sign-off, so nothing is backfilled and every
-- event created before this reads exactly as it did.

alter table public.events
  add column if not exists ask_name text;

-- 0009's function, unchanged except for ask_name in the last chained object. Copied rather than
-- retyped: the first object is already at Postgres's ceiling of 100 arguments, which is why the
-- fields are chained across several objects to begin with.
...
create or replace function public.event_public_json(e public.events) returns jsonb language sql stable as $$
  select jsonb_build_object(
    ...
    'know_order', e.know_order, 'ask_name', e.ask_name
  ) || jsonb_build_object(
