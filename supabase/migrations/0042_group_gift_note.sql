-- The host's own words for the group gift, on the gifts block.
--
-- The block writes "Everyone is going in together on one present: a big trampoline." from the
-- gift's description. True, and not how everybody would say it: a wake, a fortieth and a
-- christening each want a different sentence, and some hosts want to name the organiser in it or
-- say why.
--
-- So this replaces that sentence when it is set, and nothing is appended to it, ever. Empty, the
-- generated line stands. That is the same rule the info booth's gifts line already follows: the
-- switch fills a silence and never edits a voice. It had to be written into good-to-know.ts after
-- the group gift started adding its own sentence onto the end of whatever a host had typed, and
-- an invite came out saying the same thing twice in two voices.
alter table public.events add column if not exists group_gift_note text;

-- And out to the guests. Body lifted from 0041 unchanged, one key added.
