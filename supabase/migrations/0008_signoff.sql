-- The sign-off: the last thing on the invite.
--
-- Everything on the invite until now was information. A real card ends the way a note ends,
-- with the host saying they are looking forward to it and signing their name. The invite stopped
-- dead on "Text Gabe", which is a help line, not a goodbye.
--
-- Two columns. The message, and whether the block is there at all, the same pair every optional
-- part of the invite has.
--
-- signoff_note is null by default, which means the built-in wording rather than an empty block,
-- so nothing is backfilled and every existing event gets a sensible sign-off without being
-- touched. The name under it is the host_line the event already has.
--
-- show_signoff defaults to true because this is a part hosts are being given, not one they have
-- to go and find. A host who does not want it turns it off like any other part.

alter table public.events
  add column if not exists signoff_note text,
  add column if not exists show_signoff boolean not null default true;

-- The function below is 0007's, unchanged except for the two new keys in the chained object that
