-- About this app, switchable.
--
-- It is the block at the very end of every invite, after the host's sign-off: what this app is,
-- and a hand up if you want one of your own. That is right for Marcia's own parties and wrong for
-- a trial client, who is not handing their guests an advertisement at the bottom of their
-- invitation. The two Monsters designs were drawn without it and it has to be able to go.
--
-- Absent means on, the same reading show_signoff has, so every event that exists keeps the block
-- it has today and nobody's invite changes underneath them.
alter table public.events add column if not exists show_about boolean not null default true;

-- And out to the guests. Restated in full because a function cannot be appended to: the body
-- below is the one live in the database, with 'show_about' added beside 'show_gifts'.
